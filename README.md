...
- **Logging:** application log via Klogger on stdout, framework warnings (Ktor, HikariCP, Exposed) via SLF4J at WARN and above; both land in the journal. The timestamp inside each line is UTC (the JVM runs in UTC, see below); journald's own prefix stays in the host's zone.
